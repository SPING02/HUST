...
const float rotationSpeed = 1.0f;  // 旋转速度
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	case GLFW_KEY_LEFT:
		yRot -= 5.0f;
		rotationSpeed;  // 设置负速度表示逆时针旋转
		break;
	case GLFW_KEY_RIGHT:
		yRot += 5.0f;
		rotationSpeed;// 设置正速度表示顺时针旋转
		break;
	case GLFW_KEY_1:
...
void Draw(void)
{
	...
	//处理图形的旋转
	vmath::mat4 trans = vmath::rotate(xRot, vmath::vec3(1.0, 0.0, 0.0)) * vmath::rotate(yRot, vmath::vec3(0.0, 1.0, 0.0));
...
int main()
{
	...
	//显示操作说明
	std::cout << "The direction keys can control the rotation of graphics." << std::endl;
	std::cout << "Use the numeric keys 1, 2, and 3 to set the polygon mode to point mode, line mode, and fill mode." << std::endl;
					//数字键1，2，3设置多边形模式为点模式、线模式和填充模式。
	std::cout << "Numpad 4 turns off culling mode." << std::endl;
					//数字键4关闭剔除模式。
	std::cout << "Numpad 5 turns on culling mode and culls the back faces of the polygons." << std::endl;
					//数字键5打开剔除模式并且剔除多边形的背面。
	std::cout << "Numeric keys 6 and 7 set the polygon winding method to clockwise or counterclockwise." << std::endl;
					//数字键6，7设置多边形绕法为顺时针或者逆时针。
